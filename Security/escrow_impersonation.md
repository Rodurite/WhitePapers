...
## 2.1 Problem Statement: Deceptive Clone Escrows
...
## 2.2 Design Goals
*   **Provenance**: Make it possible to cryptographically identify escrows created by Rodurite’s canonical factory.
*   **Oracle Filtering**: Restrict Rodurite’s oracle to only process escrows that are both factory-originated and backend-registered.
*   **Frontend Safety**: Prevent users from interacting with untrusted clone escrows via the official Rodurite UI.

## 2.3 Proposed Solution: Factory Binding and Database Registration
